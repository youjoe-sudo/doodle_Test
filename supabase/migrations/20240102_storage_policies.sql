...
CREATE POLICY "Users can upload own receipts" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'receipts' AND (storage.foldername())[1] = auth.uid()::text);

CREATE POLICY "Users can view own receipts" ON storage.objects FOR SELECT USING (bucket_id = 'receipts' AND (storage.foldername())[1] = auth.uid()::text);
...
CREATE POLICY "Users can access own receipts"
  ...
  TO authenticated
  USING (bucket_id = 'receipts' AND (storage.foldername(1))[1] = auth.uid()::text);